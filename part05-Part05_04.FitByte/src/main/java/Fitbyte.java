...
the target heart rate should be about ((200-50) * (0.75) + 50), i.e., 162.5 beats per minute.
...
The proportion is given as a number between zero and one. The class should have:
...
public class Fitbyte {
    ...
    //constructor for class "Fitbyte"
    public Fitbyte(int age, int restingHeartRate) {
        this.age = age;
        this.restingHeart = restingHeartRate;
    }

    //calculates and returns the heart rate
    public double targetHeartRate(double percentageOfMaximum) {
        //calculates the maximum heart rate
        double maximum = (206.3 - (0.711 * this.age));

        //calculates and returns the target's heart rate
        return ((maximum - this.restingHeart) * percentageOfMaximum + this.restingHeart);
    }
}
